...
var GrowlService = (function () {
    ...
    function GrowlService(config) {
        this.config = config;
        this.OnAddMessage = new EventEmitter();
        this.OnRemoveMessage = new EventEmitter();
    }
    /**
     * @param {?} msg
     * @return {?}
...
GrowlComponent.decorators = [
    { type: Component, args: [{
                selector: 'ngx-growl',
                template: "<div class=\"message-container {{item.type.toLowerCase()}}\" *ngFor=\"let item of growlItems; let idx = index;\" [ngClass]=\"{closing: item.closing}\"> <span class=\"close\" (click)=\"close(idx)\">×</span> <strong class=\"heading\">{{item.heading}}</strong> <span class=\"message\">{{item.message}}</span> </div>",
...
export { GrowlModule, GrowlComponent, GrowlService };
